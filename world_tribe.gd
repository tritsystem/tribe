...
## A tribe's 3 named resources -- pure display/flavor layer on top of the
## existing single-currency economy: material_name()/material_stock stay the
## ONE thing that's actually forged, traded, and looted (unchanged -- that
## machinery is wired through MATERIAL_GEAR, gear tiers, capture_state/
## apply_state, looting, castle cost, dozens of call sites). Read from the
## archetype's .tribe file's `resources:` line (parsed for free by
## tribe_dsl.gd's existing comma-list handling); guarantees a food-ish entry
## even if a file's data is missing or malformed.
