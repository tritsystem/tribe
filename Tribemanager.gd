...
# The ONE MultiMeshInstance3D that draws the whole forest (tree_field.gd). Built
# before any tree spawns so each tree's mark_dirty() has a field to notify. Every
# tree.gd node is now an invisible gameplay record; this draws them all in ~1 call.
func _spawn_tree_field() -> void:
	if get_tree().get_first_node_in_group("tree_field") != null:
		return
	var tf := MultiMeshInstance3D.new()
